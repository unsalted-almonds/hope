...
public class WordLadder {
    ...
    public int ladderLength(String start, String end, Set<String> dict) {
        ...
        helper(start, end, dict, 1, new HashSet<String>());
    ...
    private void helper(String start, String end, Set<String> dict, int length, Set<String> visited) {

        if (differByOne(start, end)) {
            if (length + 1 < result) {
                result = length + 1;
            }
            return;
        ...
        for (String item : dict) {
            ...
            if (!visited.contains(item) && differByOne(start, item)) {
                visited.add(item);
                helper(item, end, dict, ++length, visited);
                visited.remove(item);
                length--;
            }
        }
